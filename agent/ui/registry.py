...
def stage_file(domain: str, prefix: str, stem: str, suffix: str = ".jsonl") -> Path:
    """``agent/outputs/<domain>/<任务>/<stem><suffix>``：一次构建任务的产物。

    任务名就是原来的「前缀」，只是从文件名前缀改成了目录名——不同批次的产物
    各占一个目录，不再在同一个文件夹里混着几百个文件。任务名为空时回落到这个
    项目的默认档（``<领域名>_train`` 之类），而不是某个全局常量——否则新建一个
    领域时会去读另一个项目的产物。
    """
    return task_dir(domain, prefix) / f"{stem}{suffix}"


def legacy_stage_file(domain: str, prefix: str, stem: str, suffix: str = ".jsonl") -> Path:
    """老布局：``agent/outputs/<domain>/<前缀>_<stem><suffix>``，仅在迁移前读得到。"""
    return domain_output_dir(domain) / f"{safe_path_part(prefix, default_prefix(domain))}_{stem}{suffix}"


def task_dir(domain: str, task: str) -> Path:
    """``agent/outputs/<domain>/<任务名>``。"""
    return domain_output_dir(domain) / safe_path_part(task, default_prefix(domain))
...
def default_prefix(domain: str, split: str | None = None) -> str:
    """这个项目的默认构建任务名（产物目录名），即 ``<领域名>_<split>``。
...
ARTIFACTS: tuple[Artifact, ...] = (
    ...
    Artifact("review_results", "reviewer 审核结果", "jsonl", "审核角色输出的严格 JSON 结果"),
    Artifact("review_rejected", "审核拒绝样本", "jsonl", "被 reviewer 拒绝的样本和原因"),
    Artifact("merged_final_prompt", "合并后的 final prompt", "jsonl", "阶段 6 产物"),
    Artifact("reviewed_final_prompt", "审核通过的 final prompt", "jsonl", "仅保留 reviewer decision=pass 的训练样本"),
...
_PREFIXED_PATHS: dict[str, tuple[str, str]] = {
    ...
    "review_rejected": ("review_rejected", ".jsonl"),
    "merged_final_prompt": ("merged_final_prompt", ".jsonl"),
    "reviewed_final_prompt": ("reviewed_final_prompt", ".jsonl"),
...
def artifact_path(key: str, domain: str, prefix: str) -> Path:
    ...
    新布局（任务目录）优先；只有那个文件确实还在老位置时才回退——没迁移过的
    项目照常能读、能续写，迁移之后自动切到新布局，不需要停机或改配置。
    """
    if key in _FIXED_PATHS:
        return _FIXED_PATHS[key](domain)
    stem, suffix = _PREFIXED_PATHS[key]
    current = stage_file(domain, prefix, stem, suffix)
    if current.exists():
        return current
    legacy = legacy_stage_file(domain, prefix, stem, suffix)
    if legacy.exists():
        return legacy
    return current


# 老布局里「<任务名>_<产物 stem>」这种能唯一拆开的文件，才认得出属于哪个任务。
# 拆不出来的（手工拷进来的 final_train.jsonl 之类）原样留在根目录，迁移时不动。
# 长尾优先：``multiturn_planner_trajectories.jsonl`` 比 ``planner_trajectories.jsonl``
# 长，先比对它，否则 ``..._valid_multiturn_planner_trajectories.jsonl`` 会被截成
# 一个叫 ``health_talent_valid_multiturn`` 的假任务。
_KNOWN_TAILS: tuple[str, ...] = tuple(
    sorted(
        (f"{stem}{suffix}" for stem, suffix in _PREFIXED_PATHS.values()),
        key=len,
        reverse=True,
    )
)

_TAIL_TO_KEY: dict[str, str] = {
    f"{stem}{suffix}": key
    for key, (stem, suffix) in sorted(
        _PREFIXED_PATHS.items(), key=lambda pair: -len(pair[1][0])
    )
}


def classify_legacy_file(name: str) -> tuple[str, str] | None:
    """把一个老布局文件名拆成 ``(任务名, 任务内的文件名)``；认不出返回 None。

    ``health_talent_train_planner_trajectories.jsonl`` → ``("health_talent_train",
    "planner_trajectories.jsonl")``；``.bak_20260920_125851`` 这类备份文件跟着
    本体一起搬，否则任务目录旁边会一直散着一堆备份。
    """
    for tail in _KNOWN_TAILS:
        marker = f"_{tail}"
        if name.endswith(marker):
            task = name[: -len(marker)]
            if task:
                return task, tail
            continue
...
def artifact_key_of(name: str) -> str | None:
    """任务内的文件名 → 产物 key；认不出返回 None（备份文件按本体算）。"""
    for tail, key in _TAIL_TO_KEY.items():
        if name == tail or name.startswith(f"{tail}."):
            return key
    return None


# 可以跨任务整合的产物：按行追加的数据集。统计报告、目录类产物不在其列——把
# 两份 markdown 首尾相接没有意义。
MERGEABLE_KEYS: tuple[str, ...] = (
    ...
    "no_intent_executor",
    "intent_downsampled",
    "executor_downsampled",
)

MERGEABLE_LABELS: tuple[str, ...] = tuple(ARTIFACT_BY_KEY[key].label for key in MERGEABLE_KEYS)
MERGE_DEFAULT_LABEL = ARTIFACT_BY_KEY["dedup_final_prompt"].label
_MERGE_KEY_BY_LABEL: dict[str, str] = {
    label: key for key, label in zip(MERGEABLE_KEYS, MERGEABLE_LABELS)
}

# 整合阶段的去重方式：界面上的说法 → 脚本的取值。
MERGE_DEDUP_CHOICES: dict[str, str] = {
    "按内容（默认）": "auto",
    "按 id": "id",
    "不去重": "none",
}
...
def _read_task_dir(domain: str, directory: Path) -> dict[str, Any]:
    item: dict[str, Any] = {
        "name": directory.name,
        "path": str(directory),
        "files": 0,
    ...
    for path in sorted(directory.iterdir()):
        ...
        key = artifact_key_of(path.name)
        if key and key not in item["artifacts"]:
            item["artifacts"].append(key)
    item["artifacts"].sort()
    item["legacy"] = False
    return item


def list_tasks(domain: str) -> list[dict[str, Any]]:
    """这个项目下有哪些构建任务，以及各自的产物规模。

    任务来自两处：任务目录（新布局），以及根目录里 ``<任务名>_<产物>`` 的文件
    （还没迁移的老布局）。后者标 ``legacy``，界面据此提示「可以搬进任务目录」。
    """
    root = domain_output_dir(domain)
    if not root.is_dir():
        return []

    fixed_dirs = {_FIXED_PATHS["health_plans"](domain).name}
    tasks: dict[str, dict[str, Any]] = {}

    def legacy_task(name: str) -> dict[str, Any]:
        item = tasks.get(name)
        if item is None:
            item = {
                "name": name,
                "path": str(task_dir(domain, name)),
                "files": 0,
                "bytes": 0,
                "mtime": 0.0,
                "legacyFiles": 0,
    ...
    for child in sorted(root.iterdir()):
        if child.is_dir():
            if child.name in fixed_dirs or child.name.startswith("."):
                continue
            tasks[child.name] = _read_task_dir(domain, child)
            continue

        split = classify_legacy_file(child.name)
        if split is None:
            continue
        task, inner = split
        item = legacy_task(task)
        try:
            stat = child.stat()
        except OSError:
            continue
        ...
        item["bytes"] += stat.st_size
        item["mtime"] = max(item["mtime"], stat.st_mtime)
        key = artifact_key_of(inner)
        if key and key not in item["artifacts"]:
            item["artifacts"].append(key)

    for item in tasks.values():
        item["artifacts"].sort()
    ordered = sorted(tasks.values(), key=lambda entry: (-entry["mtime"], entry["name"]))
    return ordered


def migrate_legacy_files(domain: str, tasks: list[str] | None = None) -> dict[str, Any]:
    """把老布局的产物搬进各自的任务目录。

    只搬「``<任务名>_<已知产物>``」这类认得出归属的文件，且**不覆盖**已有目标：
    目标位置已经有同名文件时留着不动并记进 ``skipped``，交给使用者判断谁更新。
    """
    root = domain_output_dir(domain)
    ...
    wanted = {safe_path_part(item, "") for item in tasks} if tasks else None
    for path in sorted(root.iterdir()):
        if not path.is_file():
            continue
        split = classify_legacy_file(path.name)
        if split is None:
            continue
        task, target_name = split
        if wanted is not None and task not in wanted:
            continue
        target = task_dir(domain, task) / target_name
        source_rel = _relative_to_root(path)
        target_rel = _relative_to_root(target)
        if target.exists():
            skipped.append({"from": source_rel, "to": target_rel, "reason": "目标已存在"})
            continue
        try:
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.move(str(path), str(target))
        except OSError as exc:  # pragma: no cover - 权限/占用这类环境问题
            skipped.append({"from": source_rel, "to": target_rel, "reason": str(exc)})
            continue
        moved.append({"from": source_rel, "to": target_rel})
...
def _relative_to_root(path: Path) -> str:
    ...
    except ValueError:  # pragma: no cover - 路径被环境变量指到项目外
        return str(path)
...
# 写入模式的两个取值。用中文当值不是偷懒：这个值只在这里和 builder 之间传递，
# 不进命令行，界面上直接显示原文比「英文值 + 中文标签」少一层映射，也就少一处
# 翻译漏掉的机会。
WRITE_MODE_APPEND = "追加（默认）"
WRITE_MODE_REBUILD = "覆盖重建"

WRITE_MODE_PARAM = Param(
    key="write_mode",
    label="写入模式",
    type="choice",
    choices=(WRITE_MODE_APPEND, WRITE_MODE_REBUILD),
    default=WRITE_MODE_APPEND,
    help="追加：只处理新样本，已有产物继续用（默认）；覆盖重建：清空产物从头跑一遍。",
    group="常用",
)


def _rebuild(values: dict[str, Any]) -> bool:
    """这个阶段这次要不要清空重来（对应脚本的 ``--force-rebuild``）。"""
    return str(values.get("write_mode") or "").strip() == WRITE_MODE_REBUILD
...
STAGES: tuple[Stage, ...] = (
    ...
    Stage(
        ...
        requires=("model",),
        cost="normal",
        params=(MODEL_PATH_PARAM, WRITE_MODE_PARAM),
    ),
    Stage(
        id="executor_answer",
    ...
    Stage(
        id="review_results",
        title="6. reviewer 质量审核",
        description="读取 executor 回答，调用审核角色输出严格 JSON 审核结果；原始 executor 文件不被修改。",
        module="agent.reviewer_generator",
        inputs=("executor_answer",),
        outputs=("review_results",),
        requires=("api",),
        cost="heavy",
        params=(
            max_samples_param("AGENT_REVIEWER_MAX_SAMPLES", 10000, "本次最多审核多少条 executor 回答。"),
            max_workers_param("AGENT_REVIEWER_MAX_WORKERS"),
            Param(key="model_name", label="审核模型", env="AGENT_REVIEWER_MODEL", fallback="跟随 executor 模型"),
            Param(key="max_retries", label="最大重试次数", type="int", env="AGENT_REVIEWER_MAX_RETRIES", fallback=5, group="高级"),
            Param(key="max_answer_chars", label="回答截断长度", type="int", env="AGENT_REVIEWER_MAX_ANSWER_CHARS", fallback=12000, group="高级"),
            Param(key="min_pass_score", label="最低通过分数", type="int", env="AGENT_REVIEWER_MIN_PASS_SCORE", fallback=3, help="低于此分数即按 reject 处理。", group="高级"),
            WRITE_MODE_PARAM,
        ),
    ),
    Stage(
        id="merged_final_prompt",
        title="7. planner/executor 合并",
    ...
    Stage(
        id="reviewed_final_prompt",
        title="8. 审核筛选 final prompt",
        description="按显式样本 ID 筛选审核通过样本，另存拒绝样本；不会覆盖 merged 原文件。",
        module="agent.filter_reviewed_sft",
        inputs=("merged_final_prompt", "review_results"),
        outputs=("reviewed_final_prompt", "review_rejected"),
        cost="light",
        params=(WRITE_MODE_PARAM,),
    ),
    Stage(
        id="dedup_final_prompt",
        title="9. final prompt 去重",
        description="MinHash LSH 近似去重，输出干净的训练集和重复簇日志。",
        module="agent.deduplicate",
        inputs=("reviewed_final_prompt",),
    ...
    # ------------------------------------------------------------------
    # 跨任务整合（编号接在主链路后面：它是主链路的收尾动作）
    # ------------------------------------------------------------------
    Stage(
        id="merge_tasks",
        title="11. 多任务整合",
        description=(
            "把多个任务里的同一类产物合并成一份（按内容去重），写进当前任务。"
            "把顶栏的任务名当成「整合结果放哪」，源任务在这里填。"
        ),
        module="agent.merge_tasks",
        inputs=(),
        outputs=(),
        category="side",
        cost="light",
        params=(
            Param(
                ...
                label="源任务",
                type="str",
                help="要合并哪些任务的产物，逗号分隔。任务名就是产物页列出的那些目录名。",
                group="常用",
                placeholder="例如 health_talent_train,smoke",
            ),
            Param(
                key="artifact",
                label="整合产物",
                type="choice",
                choices=MERGEABLE_LABELS,
                default=MERGE_DEFAULT_LABEL,
                help="合并哪一类产物；结果写进当前任务的同名文件，原有内容默认保留。",
                group="常用",
            ),
            Param(
                key="dedup_by",
                label="去重方式",
                type="choice",
                choices=tuple(MERGE_DEDUP_CHOICES),
                default="按内容（默认）",
                help="按内容：同一道题在不同的任务里也算重复；按 id：只认 id 相同的。",
                group="常用",
            ),
            WRITE_MODE_PARAM,
        ),
    ...
    Stage(
        ...
        cost="heavy",
        params=(
            Param(key="name", label="模型名字", fallback="司宁健康管理助手"),
            Param(key="team", label="团队/公司", fallback="南京邮电大学管理学院", group="高级"),
            Param(key="product", label="产品定位", fallback="司宁健康管理 agent", group="高级"),
            Param(
                key="stage",
                label="生成阶段",
                type="choice",
...
# 一条龙默认执行顺序；审核阶段位于 executor 和最终训练集之间。
...
def _build_planner_trajectories(ctx: RunContext, v: dict[str, Any]) -> list[str]:
    ...
    _flag(cmd, "--force-rebuild", _rebuild(v))
    return cmd
...
def _build_review_results(ctx: RunContext, v: dict[str, Any]) -> list[str]:
    executor, reviews = _paths(ctx, "executor_answer", "review_results")
    cmd = [
        ctx.python, "-m", "agent.reviewer_generator",
        "--domain", ctx.domain,
        "--input", str(executor),
        "--output", str(reviews),
    ]
    _opt(cmd, "--model-name", v.get("model_name"))
    _opt(cmd, "--max-samples", v.get("max_samples"))
    _opt(cmd, "--max-workers", v.get("max_workers"))
    _opt(cmd, "--max-retries", v.get("max_retries"))
    _opt(cmd, "--max-answer-chars", v.get("max_answer_chars"))
    _opt(cmd, "--min-pass-score", v.get("min_pass_score"))
    _flag(cmd, "--force-rebuild", _rebuild(v))
    return cmd
...
def _build_reviewed_final_prompt(ctx: RunContext, v: dict[str, Any]) -> list[str]:
    merged, reviews, reviewed, rejected = _paths(
        ctx, "merged_final_prompt", "review_results", "reviewed_final_prompt", "review_rejected"
    )
    cmd = [
        ctx.python, "-m", "agent.filter_reviewed_sft",
        "--merged-input", str(merged),
        "--reviews-input", str(reviews),
        "--output", str(reviewed),
...
def _build_dedup_final_prompt(ctx: RunContext, v: dict[str, Any]) -> list[str]:
    source, dedup, duplicates = _paths(
        ctx, "reviewed_final_prompt", "dedup_final_prompt", "duplicates"
    )
    cmd = [
...
def _build_merge_tasks(ctx: RunContext, v: dict[str, Any]) -> list[str]:
    """跨任务整合：把选定的几个任务里的同名产物并进当前任务。"""
    raw = str(v.get("source_tasks") or "").replace("，", ",")
    sources = [item.strip() for item in raw.split(",") if item.strip()]
    if not sources:
        raise ValueError("先填「源任务」：要合并哪些任务的产物，逗号分隔。")

    label = str(v.get("artifact") or MERGE_DEFAULT_LABEL).strip()
    key = _MERGE_KEY_BY_LABEL.get(label)
    if key is None:
        raise ValueError(f"认不出要整合的产物：{label}")
    if key not in MERGEABLE_KEYS:
        raise ValueError(f"「{label}」不支持跨任务整合。")

    target = artifact_path(key, ctx.domain, ctx.prefix)
    inputs = [artifact_path(key, ctx.domain, task) for task in sources]
    if any(path == target for path in inputs):
        raise ValueError(
            f"当前任务（{ctx.prefix}）也在源任务里，整合结果会盖掉输入。"
            "把顶栏的任务名换成「整合结果放哪」，源任务只填要合并的那些。"
        )
    if not any(path.is_file() for path in inputs):
        raise ValueError(
            f"这些任务里都没有「{label}」：{', '.join(sources)}。"
            "核对一下任务名，或者去产物页看看它们是不是还没迁进任务目录。"
        )

    cmd = [ctx.python, "-m", "agent.merge_tasks"]
    for path in inputs:
        cmd.extend(["--input", str(path)])
    cmd.extend(["--output", str(target)])
    _opt(cmd, "--dedup-by", MERGE_DEDUP_CHOICES.get(str(v.get("dedup_by") or ""), "auto"))
    _flag(cmd, "--force-rebuild", _rebuild(v))
    return cmd
...
_BUILDERS: dict[str, Callable[[RunContext, dict[str, Any]], list[str]]] = {
    ...
    "reviewed_final_prompt": _build_reviewed_final_prompt,
    ...
    "merge_tasks": _build_merge_tasks,
}
...
# ---------------------------------------------------------------------------
# 全局环境文件（agent/.env）的自由键值编辑
#
# 「运行时配置」弹窗只认 CONFIG_FIELDS 那张白名单 —— 那是模型接入、本地模型和
# 超时，结构稳定、有类型和默认值，值得逐个登记。
#
# 工具凭据是另一回事：search_web 读 EXA_API_KEY，换成别的搜索后端就是另一个键
# 名，每个项目用的工具都不一样，代码里登记不过来，登记了也等于替用户决定「这个
# 项目该用哪家搜索」。所以工具页另开一个编辑器，把整个文件按 KEY=VALUE 平铺，
# 随便加、随便删；工具侧一行都不用改，它们本来就是 os.getenv("XXX_API_KEY")。
# ---------------------------------------------------------------------------

# 合法环境变量名：字母或下划线开头，之后字母 / 数字 / 下划线。
ENV_KEY_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*$")

# 键名里出现这些词就当凭据处理：界面上不回显、留空表示不修改。
_SECRET_KEY_RE = re.compile(r"(KEY|TOKEN|SECRET|PASSWORD|PASSWD|CREDENTIAL|COOKIE)", re.IGNORECASE)


def looks_like_secret(key: str) -> bool:
    """键名像凭据吗？只看名字，不看值 —— 值不能作为判断依据（可能是占位符）。"""
    return bool(_SECRET_KEY_RE.search(key or ""))


def describe_env_file() -> dict[str, Any]:
    """工具页的环境文件视图：agent/.env 里现有的每一行。

    和 ``describe_config`` 的区别：不做白名单过滤，也不做 fallback 解析 —— 这是
    「文件里写了什么」，不是「哪一项最终生效成什么」。密钥只回显掩码。
    """
    values = read_env_file()
    entries: list[dict[str, Any]] = []
    for key, value in values.items():
        secret = looks_like_secret(key)
        # 进程环境变量压过文件时工具实际拿到的是另一份值，这里如实标出来，
        # 否则会出现「界面上配着 A、工具却用 B」这种没法自查的状态。
        override = os.environ.get(key, "")
        entries.append(
            {
                "key": key,
                "value": "" if secret else value,
                "effective": _mask_secret(value) if secret else value,
                "secret": secret,
                # 这个键在「运行时配置」弹窗里也有对应字段，两处改的是同一行。
                "managed": key in CONFIG_FIELD_BY_KEY,
                "override": bool(override) and override != value,
            }
        )
    return {
        "path": str(ENV_PATH),
...
def update_env_file(
    ...
    removals: list[Any] | None = None,
) -> dict[str, Any]:
    """自由增删改 agent/.env 的键，不查白名单。

    ``values`` 里值为空的项会删掉那一行（和运行时配置一致：空 = 不设置这个键）；
    密钥类型的键留空表示「不修改」，避免误清空。``removals`` 是明确要删掉的键名。

    写完同步 ``os.environ``：阶段子进程由本进程派生、继承这份环境，而子进程里的
    ``load_dotenv`` 默认不覆盖已存在的变量，不同步的话刚改的值对下一次运行不生效。
    """
    updates: dict[str, str | None] = {}
    ...
    for raw_key, raw_value in (values or {}).items():
        ...
        if not ENV_KEY_RE.match(key):
            raise ValueError(f"{key!r} 不是合法的环境变量名：只能由字母、数字、下划线组成，且不能以数字开头。")
        value = str(raw_value if raw_value is not None else "").strip()
        if _ENV_FORBIDDEN_RE.search(value):
            raise ValueError(f"「{key}」的值不能包含换行或空字符。")
        if looks_like_secret(key) and not value:
            continue  # 密钥留空 = 不改，和运行时配置同一套约定
        updates[key] = value or None

    for raw_key in removals or []:
        key = str(raw_key or "").strip()
        if key:
            updates[key] = None
...
# 角色先作为结构化模板资产管理。它们不会被当作任意 Python 执行，后续接入
# 流水线时只消费这里已经校验过的字段，避免前端生成不可控的导入和运行代码。
ROLE_NAME_RE = re.compile(r"^[a-z][a-z0-9_]{1,63}$")
ROLE_TYPES = ("planner", "executor", "reviewer", "custom")
...
ROLE_TEMPLATES: tuple[dict[str, Any], ...] = (
    {
        ...
        "displayName": "规划助手模板",
        "type": "planner",
        "description": "根据用户问题和可用工具规划下一步动作。",
        "systemPrompt": "你是一个规划助手。请根据用户目标、已有信息和可用工具，判断是否需要调用工具、追问或结束规划。输出必须遵守当前流水线规定的结构化协议。",
        "inputSchema": {
            "type": "object",
            "properties": {
                "user_query": {"type": "string", "description": "用户原始问题"},
                "history": {"type": "array", "description": "可选的历史对话上下文"},
                "previous_steps": {"type": "array", "description": "当前样本已经完成的 assistant/tool 规划步骤"},
                "available_tools": {"type": "array", "description": "当前这条样本可调用的 OpenAI tools schema"},
            },
        ...
        "outputSchema": {
            "type": "object",
            "properties": {
                "intent": {"type": "string", "description": "用户原始问题的整体意图"},
                "action": {"type": "string", "enum": ["tool_call", "ask_clarification", "planning_finish"]},
                "ask": {"type": "string", "description": "action=ask_clarification 时的追问建议"},
                "finish_reason": {"type": "string", "description": "action=planning_finish 时的结束原因"},
                "tool_calls": {"type": "array", "description": "action=tool_call 时由 OpenAI 原生 tool_calls 返回的调用列表"},
            },
            "required": ["intent", "action"],
        },
    },
    {
        "name": "executor_template",
        "displayName": "执行助手模板",
        "type": "executor",
        "description": "根据规划结果和参考信息生成面向用户的最终回答。",
        "systemPrompt": "你是一个执行助手。请基于用户问题、规划结果和外部参考信息生成清晰、准确、可执行的最终回答，不要暴露内部规划协议。",
        "inputSchema": {
            "type": "object",
            "properties": {
                "user_query": {"type": "string", "description": "用户原始问题"},
                "intent": {"type": "string", "description": "planner 识别出的意图（领域可关闭）"},
                "external_info": {"type": "string", "description": "工具调用结果或知识库参考信息"},
                "answer_template": {"type": "string", "description": "领域提供的回答建议模板（可选）"},
                "clarification_ask": {"type": "string", "description": "planner 生成的追问建议（可选）"},
                "compressed_history_text": {"type": "string", "description": "压缩后的历史对话（可选）"},
                "recent_history_text": {"type": "string", "description": "近期历史对话（可选）"},
            },
            "required": ["user_query", "external_info"],
        },
        "outputSchema": {
            "type": "string",
            "description": "assistant 的最终回答文本；模型返回的 reasoning_content 只作为内部元数据，不属于用户可见回答。",
        },
    },
    {
        "name": "reviewer_template",
        "displayName": "质量审核模板",
        "type": "reviewer",
        "description": "检查执行结果的相关性、完整性、准确性和安全边界。",
        "systemPrompt": "你是一个严格的质量审核员。请审核执行结果是否回答用户问题、是否完整准确、是否符合安全边界，并只输出约定的结构化审核结果。",
        "inputSchema": {
            "type": "object",
            "properties": {"user_query": {"type": "string"}, "answer": {"type": "string"}, "intent": {"type": "string"}, "external_info": {"type": "string"}},
            "required": ["user_query", "answer"],
        },
        "outputSchema": {
            "type": "object",
            "properties": {"decision": {"type": "string", "enum": ["pass", "reject"]}, "score": {"type": "integer", "minimum": 1, "maximum": 5}, "issues": {"type": "array"}, "suggestions": {"type": "array"}},
            "required": ["decision", "score", "issues", "suggestions"],
        },
    },
)
...
def validate_role_name(name: str) -> str:
    ...
    if not ROLE_NAME_RE.match(cleaned):
        raise ValueError("角色名只能用小写字母、数字和下划线，字母开头，2-64 个字符。")
    return cleaned


def roles_dir(domain: str) -> Path:
    """``agent/domains/<domain>/roles``：自定义角色按项目隔离存放。"""
    target = validate_domain_name(domain)
    root = domain_dir(target)
    if not root.is_dir():
        raise ValueError(f"项目 {target!r} 不存在。")
    return root / "roles"


def _locate_role(domain: str, name: str) -> Path:
    return roles_dir(domain) / f"{validate_role_name(name)}.json"


def role_templates() -> list[dict[str, Any]]:
    return [dict(item) for item in ROLE_TEMPLATES]


def _normalize_role_payload(payload: dict[str, Any], name: str | None = None) -> dict[str, Any]:
    target = validate_role_name(name or str(payload.get("name") or ""))
    role_type = str(payload.get("type") or "custom").strip().lower()
    if role_type not in ROLE_TYPES:
        raise ValueError("角色类型只能是 planner、executor、reviewer 或 custom。")
    display_name = str(payload.get("displayName") or payload.get("display_name") or target).strip()
    description = str(payload.get("description") or "").strip()
    system_prompt = str(payload.get("systemPrompt") or payload.get("system_prompt") or "").strip()
    if not display_name:
        raise ValueError("角色显示名不能为空。")
    if not system_prompt:
        raise ValueError("角色 system prompt 不能为空。")
    input_schema = payload.get("inputSchema", payload.get("input_schema", {}))
    output_schema = payload.get("outputSchema", payload.get("output_schema", {}))
    if not isinstance(input_schema, dict) or not isinstance(output_schema, dict):
        raise ValueError("输入协议和输出协议必须是 JSON 对象。")
    return {
        "name": target,
...
def list_roles(domain: str) -> dict[str, Any]:
    target = validate_domain_name(domain)
    root = roles_dir(target)
    items: list[dict[str, Any]] = []
    builtin_names = {item["name"] for item in ROLE_TEMPLATES}
    # 当前流水线中的三个角色先以“内置角色”显示，让用户能看到真实存在的
    # planner / executor / reviewer；它们仍可作为模板被复制成自定义角色。
    for template in ROLE_TEMPLATES:
        builtin = dict(template)
        builtin["name"] = str(template["type"])
        builtin["builtin"] = True
        builtin["template"] = template["name"]
        builtin["path"] = ""
    ...
    if root.is_dir():
        for path in sorted(root.glob("*.json")):
            try:
                ...
                item = _normalize_role_payload(payload, path.stem)
                item["builtin"] = False
                item["path"] = str(path)
                items.append(item)
            except (OSError, ValueError, json.JSONDecodeError) as exc:
                items.append({"name": path.stem, "displayName": path.stem, "error": str(exc), "path": str(path)})
    return {"roles": items, "templates": role_templates(), "domain": target, "dir": str(root), "builtinTemplates": sorted(builtin_names)}


def read_role(domain: str, name: str) -> dict[str, Any]:
    target_domain = validate_domain_name(domain)
    target = validate_role_name(name)
    path = _locate_role(target_domain, target)
    if not path.is_file():
        raise ValueError(f"角色 {target!r} 不存在。")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("角色文件必须是 JSON 对象。")
    result = _normalize_role_payload(payload, target)
    result.update({"path": str(path), "content": path.read_text(encoding="utf-8")})
    return result


def create_role(domain: str, payload: dict[str, Any], template: str | None = None) -> dict[str, Any]:
    target_domain = validate_domain_name(domain)
    requested_template = str(template or "").strip()
    role_type = str(payload.get("type") or "custom").strip().lower()
    if not requested_template and role_type in {"planner", "executor", "reviewer"}:
        requested_template = f"{role_type}_template"
    source = next((item for item in ROLE_TEMPLATES if item["name"] == requested_template), None)
    if template and source is None:
        raise ValueError(f"角色模板 {template!r} 不存在。")
    merged = {**(source or {}), **payload}
    item = _normalize_role_payload(merged)
    root = roles_dir(target_domain)
    path = _locate_role(target_domain, item["name"])
    if path.exists():
        raise ValueError(f"角色 {item['name']!r} 已存在。")
    root.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(item, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    item.update({"created": True, "path": str(path)})
    return item


def write_role(domain: str, name: str, payload: dict[str, Any]) -> dict[str, Any]:
    target_domain = validate_domain_name(domain)
    target = validate_role_name(name)
    root = roles_dir(target_domain)
    path = _locate_role(target_domain, target)
    if not path.is_file():
        raise ValueError(f"角色 {target!r} 不存在。")
    item = _normalize_role_payload(payload, target)
    previous = path.read_text(encoding="utf-8")
    content = json.dumps(item, ensure_ascii=False, indent=2) + "\n"
    if previous != content:
        backup = _backup_file(root, path.name, previous)
        path.write_text(content, encoding="utf-8")
    else:
        backup = ""
    item.update({"changed": previous != content, "backup": backup, "path": str(path)})
    return item


def delete_role(domain: str, name: str) -> dict[str, Any]:
    target_domain = validate_domain_name(domain)
    target = validate_role_name(name)
    path = _locate_role(target_domain, target)
    if not path.is_file():
