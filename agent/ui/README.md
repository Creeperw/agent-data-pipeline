...
**输出前缀**决定产物文件名。默认由领域名和数据集拼成，即 `<领域名>_<数据集>`，
例如 `health_talent_train_planner_trajectories.jsonl`。前缀里带项目名，是为了让
同一台机器上多个项目的数据即使被收集到一起也认得出出处，训练集和测试集也不会
互相覆盖。

输入框右边的两个胶囊按钮是领域包声明的数据集快捷入口——「训练数据」对应
`<领域名>_train`、「测试数据」对应 `<领域名>_valid`。点一下就把前缀换过去，当前
落在哪一档会高亮。前缀也可以手填（例如跑 `smoke` 试验），手填的值在切换领域时
原样保留；只有当它正好是上一个领域声明的某一档时，才会被换成新领域的同一档。
...
| 产物命名 | `--prefix`（不传则 `<领域名>_train`） | 顶栏「输出前缀」（同一套规则） |
