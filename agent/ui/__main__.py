...
def main() -> None:
    ...
    # 记下启动命令行，供界面上的「重启」原地重放（改完 agent/core/** 后要用）。
    # 只有这里知道当初是怎么启动的：重启就是「把这条命令再执行一遍」。
    try:
        from .restart import record_launch
    except ImportError:  # pragma: no cover - 直接执行本文件时没有包上下文
        from agent.ui.restart import record_launch  # type: ignore
