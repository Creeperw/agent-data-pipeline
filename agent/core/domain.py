...
class DomainSpec:
    ...
    def get_reviewer_system_prompt(self) -> str:
        """Return the reviewer contract, with a safe domain-neutral fallback."""

        return self.reviewer_system_prompt or (
            "你是一个训练数据审核员。根据用户问题、参考信息和执行阶段回答，"
            "判断回答是否相关、完整、准确、可执行且符合领域安全边界。"
        )
